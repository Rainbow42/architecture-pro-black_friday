...
По заданию 7 выбрал три ключа шардирования: `{ category: 1, _id: 1 }` для товаров, `{ customer_id: "hashed", created_at: 1 }` для заказов и `{ owner_id: "hashed" }` для корзин. Для корзин при этом введено общее поле владельца, которое сводит два разных пути поиска к одному запросу на один шард.

По заданию 8 главный вывод такой: балансировщик выравнивает объём данных, но не нагрузку. Значит, перекос по запросам надо ловить своими метриками — прежде всего долей операций на шард и тем, как запросы распределяются по значениям ключа. Лечится зонами, ручным переносом фрагментов, кешем и, в крайнем случае, перекладкой коллекции.

По заданию 9 границу провёл так: всё, что покупатель только что изменил, и всё, что связано с деньгами и остатками, читаем с первичного узла. Каталог, описания и старую историю — со вторичных.

По заданию 10 в Cassandra уходят история заказов, история их состояний, корзины и поведенческие события, а остатки и каталог остаются в MongoDB. Ключи разделов сделал на идентификаторах с большим числом значений, а там, где раздел может разрастись, добавил «ведро» по месяцу. Тогда при добавлении узлов данные не переезжают целиком.
