...
| `{ category: 1, _id: 1 }` | Взяла это. По `category` — на нужные шарды; `_id` дробит «Электронику», чтобы не лежала одним куском |

Цену в ключ не брала: она меняется. Фильтр по цене — индекс `{ category: 1, price: 1 }`. В запросах по `_id` передавать `category`.
...
| `{ customer_id: "hashed", created_at: 1 }` | Взяла это. Хеш раскидывает покупателей; история — один шард; `created_at` даёт «последние N» без сортировки в памяти |
...
| `{ owner_id: "hashed" }` | Взяла это. Оба пути — на один шард |
...
Контекст: 70 % запросов на «Электронику» — перегруз одного шарда. Балансировщик выравнивает данные, не нагрузку.
...
| `products` | Остаток в карточке | `secondaryPreferred` | 5 с | Покупателю достаточно примерно верного числа |
...
| `orders` | Статус позже | `primaryPreferred` | 5 с | Страница должна открыться даже если primary недоступен |
...
| `carts` | Брошенные корзины для аналитики | `secondary` | 5 мин | На покупателя не влияет |
...
Категорию или геозону ключом раздела брать не стоит — получится та же «Электроника» на одних узлах. Мелкие партиции дают добавление узлов без полной перекладки (в отличие от шардирования по диапазонам в MongoDB).

### 10.3. Восстановление целостности

| Сущность | Основное | Дополнительно | Согласованность | Почему |
| --- | --- | --- | --- | --- |
| Заказы | Плановая сверка раз в неделю | подсказки + починка при чтении | запись и чтение `LOCAL_QUORUM` | Терять заказ нельзя |
...
| Сессии / просмотры | Подсказки при недоступности | починка при чтении изредка | `LOCAL_ONE` | Скорость важнее полной сходимости копий |
